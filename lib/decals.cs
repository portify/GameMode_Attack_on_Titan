if(isFile("Add-Ons/System_ReturnToBlockland/server.cs")) {
  ...
  RTB_registerPref("Decal Limit", "Decal System", "$DecalSystem::DecalLimit", "int 0 2000", "Support_Decals", 200, 0, 1);
  RTB_registerPref("Decal Timeout (MS)", "Decal System", "$DecalSystem::DecalTimeout", "int -1 300000", "Support_Decals", 300000, 0, 1);
} 
else {
...
function serverCmdClearDecals(%this) {
  ...
  if (!isObject(DecalGroup)) {
    ...
    %decals = DecalGroup.getCount() + DecalGroupNew.getCount();
  }
